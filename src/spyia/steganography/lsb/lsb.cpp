...
std::string Lsb::generateHeader(int position, int maxFiles, Spyia::Encryption::EncryptionType encryptionType, const std::string &iv) const
{
    // header example: f:1/3-AESCBC:iv-LSB
    return "f:" + std::to_string(position) + "/" + std::to_string(maxFiles) + "-" + encryptionToString(encryptionType) + ":" + iv + "-LSB";
}
